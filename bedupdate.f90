...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    module bedupdate_module
        ...
        use sediment_module, only : morstart,morfac,struct,gmax,sourcesink,por, & !dzbdt add to sediment_module
            thick,toler,nd_var,trim,eps,totalthick,ero1,depo_ex1
                    ...
                    do j=2,my
                        do i=2,mx
                            ...
                            if (gmax==1) then ! Simple bed update in case one fraction
                                ...
                                call update_fractions(i,j,totalthick(i,j),pbbed(i,j,:,:),sum(dzg(i,j,:),edg(i,j,:)/sum(edg(i,j,:),dt,pb)
                                pbbed(i,j,:,:)=pb
                                !edg(i,j,:) = dzg(i,j,:)*(1.0-por)/dt
                    ...
                    do i=1-mbc,mx+mbc-1
                        do j=1-mbc,my+mbc
                            ...
                            if(abs(dzbdx(i,j))>dzmax .and. h(i+nint(max(0.0,sign(one,dzbdx(i,j)))),j)>eps) then
                                ...
                                dzb=sign(1.0,dzbdx(i,j))*(min(abs(dzbdx(i,j))-dzmax,thick))*dx !limit to the first layer
                                if (dzb >= 0.0) then
                    ...
                    do j=1-mbc,my+mbc-1 !
                        do i=1-mbc,mx+mbc
                            ...
                            if(abs(dzbdy(i,j))>dzmax .and. h(i,j+nint(max(0.0,sign(one,dzbdy(i,j)))))>eps) then
                                aval=.true.
                                dzb=sign(1.0,dzbdy(i,j))*(min(abs(dzbdx(i,j))-dzmax,thick))*dy ! limit to the first layer
                                if (dzb >= 0.0) then
                ...
                if (deltath_o > deltath_d)
                    if (nlf == 1) then
                        ...
                        do j =1, gmax
                            ...
                            pbt(1,j) = (pbs(i,j)*thick - dps(j)*deltath_d+pbs(2,j)*deltath_d)/thick
                        end do
                        if (nlf > 2) then
                            do i = 2, nlf-1
                                do j =1, gmax
